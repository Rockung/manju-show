import mditr from './regexp';
import { getAttrs, getAttrsString } from './attrs/utils';
...
export default md => {
  ...
  md.core.ruler.push('img_blockify', function (state) {
    ...
    for (let blkIdx = 0; blkIdx < state.tokens.length; blkIdx++) {
      ...
      let paragraphOpens = 0;
      for (let i = blkIdx + 1; i < state.tokens.length; i++) {
        ...
        let prevBlkToken = state.tokens[i - 1];
        if (prevBlkToken.type !== 'inline') {
          break;
        }
        // console.log(prevBlkToken.content)
        let prevInlineToken = prevBlkToken.children[prevBlkToken.children.length - 1];
